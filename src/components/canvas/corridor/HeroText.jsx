...
const HeroText = ({ position = [0, 0.3, 0] }) => {
    ...
    // Letter positions for PRASHANT split effect
    const letters = useMemo(() => [
        { char: 'P', baseX: -1.05, splitDir: -2.0, delay: 0 },
        { char: 'R', baseX: -0.75, splitDir: -1.4, delay: 0 },
        { char: 'A', baseX: -0.45, splitDir: -0.9, delay: 0 },
        { char: 'S', baseX: -0.15, splitDir: -0.4, delay: 0 },
        { char: 'H', baseX: 0.15, splitDir: 0.4, delay: 0 },
        { char: 'A', baseX: 0.45, splitDir: 0.9, delay: 0 },
        { char: 'N', baseX: 0.75, splitDir: 1.4, delay: 0 },
        { char: 'T', baseX: 1.05, splitDir: 2.0, delay: 0 },
    ], []);

    // Tagline words for split effect
    const taglineWords = useMemo(() => [
        { text: '<', baseX: -0.7, splitDir: -1.4, delay: 0 },
        { text: 'ai', baseX: -0.3, splitDir: -0.7, delay: 0 },
        { text: 'developer', baseX: 0.3, splitDir: 0.7, delay: 0 },
        { text: '/>', baseX: 0.7, splitDir: 1.4, delay: 0 },
    ], []);
    ...
    useFrame((state, delta) => {
        ...
        // Apply split to each letter of PRASHANT
    ...
    return (
        <group ref={groupRef} position={position} scale={[scale, scale, 1]}>
            {/* PRASHANT Letters - Rubik Scribble font with fade-in animation */}
            ...
            <SmallStar position={[-1.3, 0.55, 0]} scale={0.07} />
            <SmallStar position={[1.3, 0.45, 0]} scale={0.05} />
            <SmallStar position={[-1.15, -0.6, 0]} scale={0.04} />
            <SmallStar position={[1.15, -0.55, 0]} scale={0.035} />
        </group>
    );
};
